...
export const banServerApp = async (
  req: Request,
  serverId: string,
  kickee: string,
  ...
  try {
    ...
    const kickeeUser = await appService.findById(kickeeApp[0].app_id);
    if (kickeeUser.length === 0) {
      throw new BadRequestException('No such app in database');
    }
    const isBanned = await serversService.findBan(serverId, kickee);
    if (isBanned.length > 0) {
      throw new BadRequestException('App is already banned');
    }

    await Promise.all([
      serverAppsService.removeApp(serverId, kickee),
      serversService.createBan(serverId, kickee),
    ]);

    return {
      valid: true,
      message: 'Banned App From Server.',
    };
  } catch (error) {
    if (
    ...
    console.log('Error in banning app from server.: ', error);
