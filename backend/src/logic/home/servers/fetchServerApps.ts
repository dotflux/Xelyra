...
export const fetchServerApps = async (
  req: Request,
  id: string,
  ...
  try {
    ...
    const serverApps = await serverAppsService.fetchBatch(id, limit, afterId);
    const appIds = serverApps.map((m: any) => m.app_id);
    ...
    console.log('Error in fetching server apps: ', error);
