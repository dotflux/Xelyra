...
export const kickServerApp = async (
  req: Request,
  serverId: string,
  kickee: string,
  ...
  try {
    ...
    await serverAppsService.removeApp(serverId, kickee);

    return {
      valid: true,
      message: 'Kicked App From Server.',
    };
  } catch (error) {
    if (
    ...
    console.log('Error in kicking app from server.: ', error);
