...
export const cancelRequest = async (
  ...
  try {
    ...
    // Check if sent request exists in new requests_sent table
    const request = await usersService.findRequestSent(user[0].id, recieverId);
    if (request.length === 0) {
      throw new BadRequestException('No such request in database');
    }
    await Promise.all([
      usersService.removeRequest(user[0].id, recieverId),
      usersService.removeRequestSent(user[0].id, recieverId),
    ]);
