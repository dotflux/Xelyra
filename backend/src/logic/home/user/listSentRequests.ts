...
export const listSentRequests = async (
  ...
  try {
    ...
    // Fetch sent requests from new requests_sent table using a public service method
    const recieverIds = await usersService.getRequestsSentBySenderId(
      user[0].id,
    );
    ...
    for (const recieverId of recieverIds) {
      const friend = await usersService.findById(recieverId);
